export const RailsSkills = [
  {
    title:"Basic Ruby on Rails",
    description:"Experience with Ruby on Rails. Familiar with MVC pattern. Rails Routing. ActiveRecord migration.",
    img_src:"https://achievement-images.teamtreehouse.com/badges_SimpleFacebook_Stage1.png",
    id: "rails-1",
    demo: "http://www.github.com"
  },
  {
    title: "Rails as API",
    description: "Familiar with using Rails to render REST API in JSON.",
    img_src: "https://achievement-images.teamtreehouse.com/badges_api_Stage3.png",
    id: "js-3",
    demo: ""
  },
  {
    title: "Ruby GEM",
    description: "Experience implementing popular ruby GEMs: Devise for user authentication; Paperclip and aws-s3 for file uploads; Stripe for payment processing; Sidekiq for background processing; ElasticSearch.",
    img_src: "https://achievement-images.teamtreehouse.com/badges_ruby_rubygems_Stage1.png",
    id: "js-4",
    demo: "",
  }
];

export const ReactSkills = [
  {
    title:"React",
    description:" Experiece creating react components.",
    img_src:"https://achievement-images.teamtreehouse.com/badges_JavaScript_react_Stage1-21.png",
    id: "rails-1"
  },
  {
    title:"React Router",
    description:"Experiece with react router.",
    img_src:"https://achievement-images.teamtreehouse.com/badges_JavaScript_react_router_Stage1.png",
    id: "rails-1",
    demo: "https://github.com/johnshencanada/portfolio"
  },
  {
    title:"Redux",
    description:"Experiece using Redux as state management for React.",
    img_src:"https://achievement-images.teamtreehouse.com/badges_JavaScript_react_redux_Stage1.png",
    id: "rails-1"
  },
];

export const JSSkills = [
  {
    title:"JavaScript",
    description:"Experience with JavaScript. Familiar with ES6 syntax such as 'arrow function and spread operator' ",
    img_src:"https://achievement-images.teamtreehouse.com/badges_JavaScript_Basics_Stage1.png",
    id: "js-1"
  },
  {
    title: "jQuery",
    description: "Some knowledge in jQuery.",
    img_src: "https://achievement-images.teamtreehouse.com/badges_JavaScript_jQueryBasics_Stage1.png",
    id: "js-3"
  },
  {
    title: "AJAX",
    description: "rednering with AJAX.",
    img_src: "https://achievement-images.teamtreehouse.com/badges_JavaScript_AJAXBasics_Stage3.png",
    id: "js-4"
  },
  {
    title: "Node.js",
    description: "Basic understanding of Node.js, Using Express.js to for server ",
    img_src: "https://achievement-images.teamtreehouse.com/badges_JavaScript_nodeBasics_Stage1.png",
    id: "js-6"
  }
];

export const CSSCourses = [

  {
    title: "Basic HTML5",
    description: ".",
    img_src: "https://achievement-images.teamtreehouse.com/bagdes_html_howtobuildawebsite_stage02.png",
    id: "html-1"
  },
  {
    title: "Bootstrap",
    description: "Experience using twitter bootstrap.",
    img_src: "https://achievement-images.teamtreehouse.com/badge_bootstrap-4-basics_stage01.png",
    id: "css-6"
  },
  {
    title: "Responsive Layouts",
    description: "Experience using CSS3 media query for responsive designs. ",
    img_src: "https://achievement-images.teamtreehouse.com/badges-CSS-ResponsiveLayouts-stage3.png",
    id: "css-3"
  }
  ];
  
export const HTMLCourses =[

  {
    title: "HTML Forms",
    description: "The web is a two-way communication medium. There’s lots of HTML elements for displaying data and producing output, and conversely, there’s also lots of HTML elements for accepting input. Accepting input from the user means creating web forms. In this course, we’ll learn about all the most important form elements that web professionals use on a daily basis.",
    img_src: "https://achievement-images.teamtreehouse.com/badges_html_forms_stage1.png",
    id: "html-2"
  }
  ];
